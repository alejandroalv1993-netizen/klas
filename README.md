...
5. Inserta tu email como moderador:

```sql
insert into public.moderator_emails (email)
values ('tu-email@dominio.com')
on conflict (email) do nothing;
```

6. En Authentication > URL Configuration configura `https://klas-apuntes.netlify.app` como Site URL.
7. Añade `http://localhost:3000/auth/callback` y `https://klas-apuntes.netlify.app/auth/callback` en Redirect URLs.
...
- Dashboard privado en `/owner/moderacion` para aprobar, rechazar, bloquear hashes y resolver reportes.
